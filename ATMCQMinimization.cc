...
ATMCQMinimization::ATMCQMinimization()
{
  ...
  //For the moment we pass these parameters when the minimizer is called
  //Default parameters
  fGain            = 4.0;
  fCoefL           = 0.025;
  fCoefT           = 0.010;
  
  //These parameters will be provided by the user
  ...
  protons            = 0;
  nucleons           = m;
  restmass           = nucleons*931.49432;
  esm                = protons*1.75879e-3*0.510998918/restmass;// ![e/m electron cm**2/(Volt*nsec**2] this is not the energy/mass but charge/mass
...
Bool_t ATMCQMinimization::MinimizeOptMapAmp(Double_t* parameter, ATEvent *event, TH2Poly* hPadPlane, const multiarray& PadCoord)
{
            ...
            if(fParticleAZ  .size()>0){
               ...
               protons    = fParticleAZ.at(0).second;
               nucleons   = m;
               restmass   = nucleons*931.49432;
               esm        = protons*1.75879e-3*0.510998918/restmass; // [e/m electron cm**2/(Volt*nsec**2] charge/mass
               std::cout<<cGREEN<<" Particle  A : "<<m<<"  -  Z : "<< protons <<cNORMAL<<std::endl;
            }else std::cerr<<cRED<<" ATMCQMinimization::MinimizeOptMapAmp -  Warning ! Particle (A,Z) not found. Using A : "<<m<<"  -  Z : "<< protons <<cNORMAL<<std::endl;
	     ...
	     */
           ...
           for (size_t i = 0; i < npadtotal; i++) {
            ...
            //TODO: Pass these parameters with the fPar pointer
            Double_t ymax    = 0.0;
            Double_t e0sm    = 0.0;
          ...
          */
            ...
            double Qtracktotal; // total charge of track
            double QMCtotal;
            double CHi2fit;
...
void ATMCQMinimization::MCvar( double* parameter, int & MCmode, int & iconvar, double & x0MC, double & y0MC, double & z0MC, double & thetaMC, 
	...
	parameter[1] = initial y position
	parameter[2] = initial z potition
	parameter[3] = initial time bin
	parameter[4] = initial phi (azimuthal angle)	    
        parameter[5] = initial radius of curvature
        parameter[6] = initial theta (trajectory angle with respect to z-axis)
        parameter[7] = initial hit ID
	*/

            if(MCmode == 1){ // initiatialisation
...
/*              std::cout << "indside mc MCmode "<< MCmode << "  iconvar " <<iconvar<<std::endl;
                ...
                std::cout << "indside mc a " <<thetaMCv<< std::endl;
...
void ATMCQMinimization::QMCsim(double* parameter, double* Qsim, double *zsimq, double & QMCtotal, double x0MC, double y0MC, double z0MC, 
   ...
   // Set these angles to 0 as the detector is no longer tilted
      thetaPad     = 0.0;
      thetaLorentz = 0.0;
      thetaRot     = 0.0;
      thetaTilt    = 0.0;	
   ...
   // filling arrays (pads) with charge 0
   ...
   // initial velocity calculation  
      ...
      Double_t beta0 = sqrt(2.*e0/(nucleons*931.49)); // velocity/c
   ...
   // storing initial conditions for x, y, and z
      ...
      Double_t v0 = beta0*29.9792;                  // initial velocity in cm/ns converted from beta = v/c | c = 29.9792 cm/ns
      Double_t dt = dzstep/(v0*TMath::Cos(theta0)); // adjusted timestep based on trajectory and velocity of target
      	       dt = dt/(Float_t)integrationsteps;   // divide timestep by how many integration steps per timestep
      Double_t  t = -dt;

   //*********************************************************************************         

   // initial velocity vector components in Cartesian from Spherical Coordinates
   ...
   //***************** Integration *******************//
      ...
      Double_t integrationmax = 10000; // max integration steps
      Int_t integration = 0;
      Int_t iterd = 0;
      Int_t iterCorr = 0;
      ...
      for(iterd; iterd<integrationmax;iterd++){
            
	    integration++;
            Float_t factq = 1.0; // quenching factor              

       	 // storing x, y, and z components in cm from mm
            xcmm[iterd] = x*10.0;
            ycmm[iterd] = y*10.0;
            zcmm[iterd] = z*10.0;
      		

            /////////////////////////// Not sure if I can remove ///////////////////////////////////////
            ...
            /**************This code is required for a tilted detector *******************************
	    ...
	    */	
  	    ...
  	    xpad[iterd] = xcmm[iterd];
            ...
            if(iterd==0) iterCorr_0 = iterCorr; //Offset renomarlization
         ...
         //*************** Update Equations for Particle Motion *******************//

         // updating particle acceleration using esm (charge/mass ratio), velocity, and _B (magnetic field in z direction)
         // governed by F = q*v x B | a = q/m*v x B
            Double_t ddxddt = esm*_B*10.*dydt*factq;     // factq = ionization quenching factor
            Double_t ddyddt = -esm*(_B*10.*dxdt)*factq;
            Double_t ddzddt = 0.;                        // B field is in z-direction; therefore, there is no force acting on this component 
	 ...
	 // updating velocity vector
 	 ...
 	 // Change in the total range of the particle
            Double_t drange  = TMath::Power((dxdt*dt + 0.5*ddxddt*TMath::Power(dt,2)),2 );
                     drange += TMath::Power((dydt*dt + 0.5*ddyddt*TMath::Power(dt,2)),2);
            	     drange += TMath::Power((dzdt*dt + 0.5*ddzddt*TMath::Power(dt,2)),2);
            	     drange  = TMath::Sqrt(drange);
	 ...
	 // updating kinetic energy and calculating velocity ratio v0/v 	
	    ...
	    ekin = ekin-sloss;                       // loss of kinetic energy
            Double_t vratio2 = ekin/ekin0;	     // (v0/v)^2
            Double_t vratio  = TMath::Sqrt(vratio2); // v0/v
	 ...
	 //**************************************************
	 // adjusting velocity to account for energy loss
            dxdt = dxdt*vratio; 
            dydt = dydt*vratio;
            dzdt = dzdt*vratio;
		
	 // updating time step based on updated dzdt to account for energy loss
	    dt = dzstep/(dzdt)/(Float_t)integrationsteps; // divide by integration steps (how many integrations per timestep)
	 ...
	 // std::cout<<" z :"<<z<<" dt : "<<dt<<" dzstep : "<<dzstep<<" dzdt : "<<dzdt<<std::endl;
      ...
      for(iterd=0;iterd<integration;iterd++){
            ...
            double costheta=cos(thetaMC);
            double sintheta=sin(thetaMC);
	    ...
	    for (int isig=0; isig<4; ++isig){
		     ...
		     // Double_t qBin = fPadPlane->GetBinContent(pBin);
                  ...
                  } // END OF PHI LOOP (iphi)

            } // END OF STRAGGLING LOOP (isig)
      ...
      } // END OF SECOND INTEGRATION LOOP
   ...
   // calculate normalized track and define the simulated track
      ...
      for (Int_t i = 0; i < npadtotal; i++) {
         ...
         //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            Qsim[i] = Qsim[i]*qsimnorm; // normalized: attention Zsim was multiplied by the unnormalized
	    ...
	    if(Qsim[i] > tracksimthreshold){ // threshold after renormalization
            ...
            } // normalized (if statement)

      } // END OF LOOP for tracksimthreshold (i)
...
void ATMCQMinimization::BackwardExtrapolation()
{
       ...
       Double_t e0ll = fEnerMin*nucleons;
       Double_t beta2 = 2.*e0ll/(nucleons*931.49);
       Double_t ekin = e0ll;
	 ...
	 //*************** Update Equations for Particle Motion *******************//
         ...
         // updating particle acceleration using esm (charge/mass ratio), velocity, and _B (magnetic field in z direction)
         // governed by F = q*v x B | a = q/m*v x B
            Double_t ddxddt = esm*B*10.*dydt*factq;     // factq = ionization quenching factor
	 ...
	 // energy loss
            ...
            Double_t beta = vcin/29.979; // !v/c
         // ecinsm=mev_m/2.*(beta)**2*1./sqrt(1.-beta**2) !relativistic
            Double_t ekin0 = nucleons*931.494*0.5*TMath::Power(beta,2); // !nonrelativistic


	 // updating kinetic energy and calculating velocity ratio v0/v 	
            sloss = sloss*fDensMin*drange;           // energy loss with density and step
	    ekin = ekin+sloss;                       // loss of kinetic energy
            Double_t vratio2 = ekin/ekin0;	     // (v0/v)^2
            Double_t vratio  = TMath::Sqrt(vratio2); // v0/v
...
/*
   ...
   Double_t y_cmm = y_sol - z_cmm*TMath::Sin(fThetaLorentz)*TMath::Cos(fThetaRot);
   */
   PosIniCmm.SetXYZ(x,y,z);//PosIniCmm.SetXYZ(x_cmm,y_cmm,z_cmm);
...
/*
  ...
  Double_t zpad = zdet;
*/
  InvPosIni.SetXYZ(x,y,z); //InvPosIni.SetXYZ(xpad,ypad,zpad);
...
Double_t ATMCQMinimization::GetEloss(Double_t c0,std::vector<Double_t>& par)
{
   if(par.size()==11){
   	...
   	std::cerr<<cRED<<"ATMCQMinimization::GetEloss -  Warning ! Wrong number of parameters."<<std::endl;
   return 0;
 }
}

Double_t ATMCQMinimization::GetEnergyFromRange(Double_t range,std::vector<Double_t>& par)
{
    ...
    if(par.size()==7)
    {
      ...
      std::cerr<<cRED<<"ATMCQMinimization::GetEnergyFromRange -  Warning ! Wrong number of parameters."<<std::endl;
      return 0;
