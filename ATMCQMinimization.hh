      ...
      protected:
       ...
       // New MC Amplitude functions
          void MCvar( double* parameter, int & MCmode, int & iconvar, double & x0MC, double & y0MC, double & z0MC,
                      double & thetaMC, double & phiMC, double & Bmin, double & dens, double & romin,
                      double & x0MCv, double & y0MCv, double & z0MCv, double & thetaMCv, double & phiMCv, double & Bminv,
...
bool  ATMCQMinimization::MinimizeGen(Double_t* parameter,T* event,const std::function<std::vector<R>*()>& func,TH2Poly* hPadPlane,const multiarray& PadCoord)
{
          ...
          for(Int_t i=0;i<HitArray->size();i++){
           ...
           MCvar(parameter, icontrol,iconvar,x0MC, y0MC, z0MC,thetaMC,phiMC, Bmin, fDens,romin,x0MCv, y0MCv,z0MCv,thetaMCv, phiMCv, Bminv, densv, rominv); // for initialisation
               ...
               for (imc1=0;imc1<imc1max;imc1++){
                 iconvar = imc1; //this controls the step of MC in MCvar
                   for (imc2=0;imc2<imc2max;imc2++){
                     icontrol=2;
                       //fPadPlane->Reset(0);
                       MCvar(parameter,icontrol,iconvar,x0MC, y0MC, z0MC, thetaMC,phiMC, Bmin, fDens,romin,x0MCv, y0MCv,z0MCv, thetaMCv, phiMCv, Bminv, densv, rominv); // for MC variation with same starting value as before
                     ...
                     if(CHi2fit<Chimin){
                       ...
                       icontrol=3;
                       MCvar(parameter,icontrol,iconvar,x0MC, y0MC, z0MC, thetaMC,phiMC, Bmin,fDens,romin,x0MCv, y0MCv,z0MCv, thetaMCv, phiMCv, Bminv, densv, rominv);
               ...
               QMCsim(parameter, Qsim, zsimq, QMCtotal,x0MC, y0MC, z0MC,  thetaMC, phiMC, Bmin,fDens,romin,e0sm,PadCoord,fPadPlane); // simulation with Chimin parameters
